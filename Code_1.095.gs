...
if (rowNum > 0) {
  ...
  let currentEnd = normalizeYmd_(p.data_fine) || String(p.data_fine || "").slice(0, 10);

  // Se stai spostando l'ultima seduta (o comunque una seduta sull'ultimo giorno),
  // aggiorna la scadenza del paziente/terapia in modo coerente con l'UI (drag sul calendario).
  try {
    const upd = updatePatientEndAfterMove_(pr, fromDate, toDate);
    if (upd) currentEnd = upd;
  } catch (e) {}
