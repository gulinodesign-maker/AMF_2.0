...
function upsertTherapiesForPatient_(userId, patientId, payload) {
  ...
  for (let i = 1; i < valuesT.length; i++) {
    ...
    existing.push({ id: rid, rowIndex: i + 1 });
  }
  ...
  function updateRow_(rowIndex, t) {
    ...
    if (col("isDeleted") > 0) shT.getRange(rowIndex, col("isDeleted")).setValue(false);
  ...
  if (Array.isArray(arr) && arr.length) {
    arr.forEach((t) => {
      const incomingId = String(t?.id || t?.therapy_id || "").trim();
  ...
  existing.forEach(o => {
    if (!keepIds.has(o.id)) toDeleteRowIdx.push(o.rowIndex);
  });
