...
function createPatient_(userId, payloadJson) {
  ...
  forceUpdateSocietaOnTherapiesForPatient_(userId, id, payload.societa_id || payload.societaId);
return { id };
}
...
function updatePatient_(userId, patientId, payloadJson) {
  ...
  forceUpdateSocietaOnTherapiesForPatient_(userId, String(patientId), payload.societa_id || payload.societaId);
...
function withCors_(output) {
  // Apps Script ContentService TextOutput may not support setHeader() in some runtimes.
  // Keep responses working even when headers can't be set.
  try {
    ...
    // No-op: return output without CORS headers
  }
  return output;
}
...
function doOptions(e) {
  // Risposta per eventuali preflight CORS (alcuni client possono inviarli)
...
// Forza la sovrascrittura della societa_id su TUTTE le terapie del paziente (anche in caso di ID terapia mancanti/variati)
function forceUpdateSocietaOnTherapiesForPatient_(userId, patientId, societaId) {
  ...
  const headersT = valuesT[0] || [];
  const idxPid = headersT.indexOf("paziente_id");
  const idxUser = headersT.indexOf("utente_id");
  const idxSoc = headersT.indexOf("societa_id");
  const idxDel = headersT.indexOf("isDeleted");
  const idxUpd = headersT.indexOf("updatedAt");

  if (idxPid < 0 || idxSoc < 0) return;
  ...
  for (let i = 1; i < valuesT.length; i++) {
    ...
    if (idxUser >= 0) {
      const u = String(r[idxUser] || "").trim();
      if (u && String(userId) && u !== String(userId)) continue;
    }
    if (idxDel >= 0 && String(r[idxDel] || "").toLowerCase() === "true") continue;

    // Sovrascrivi sempre
    shT.getRange(i + 1, idxSoc + 1).setValue(sid);
    if (idxUpd >= 0) shT.getRange(i + 1, idxUpd + 1).setValue(now);
  }
}
